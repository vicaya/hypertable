...
public class RequestHandlerAttrGet extends ApplicationHandler {

    static final Logger log = Logger.getLogger("org.hypertable.Hyperspace");

    public RequestHandlerAttrGet(Comm comm, Hyperspace hyperspace, Event event) {
	super(event);
	mComm = comm;
    ...
    public void run() {
	ResponseCallbackAttrGet cb = new ResponseCallbackAttrGet(mComm, mEvent);
	...
	try {
	    ...
	    mHyperspace.AttrGet(cb, fileName, attrName);

	}
	catch (ProtocolException e) {
	    int error = cb.error(Error.PROTOCOL_ERROR, e.getMessage());
	    log.severe("Protocol error (ATTRGET) - " + e.getMessage());
	    if (error != Error.OK)
		log.severe("Problem sending (ATTRGET) error back to client - " + Error.GetText(error));
	}
    }
