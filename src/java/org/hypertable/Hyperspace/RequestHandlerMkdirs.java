...
public class RequestHandlerMkdirs extends ApplicationHandler {
    ...
    public RequestHandlerMkdirs(Comm comm, Hyperspace hyperspace, Event event) {
	super(event);
	mComm = comm;
	...
	try {
	    ...
	    if ((dirName = Serialization.DecodeString(mEvent.msg.buf)) == null)
		throw new ProtocolException("Directory name not properly encoded in request packet");

	    mHyperspace.Mkdirs(cb, dirName);
	}
