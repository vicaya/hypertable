...
public class RequestHandlerAttrSet extends ApplicationHandler {

    static final Logger log = Logger.getLogger("org.hypertable.Hyperspace");

    public RequestHandlerAttrSet(Comm comm, Hyperspace hyperspace, Event event) {
	super(event);
	mComm = comm;
	...
	String  fileName, attrName, attrValue;
	...
	try {
	    ...
	    log.severe("Protocol error (ATTRSET) - " + e.getMessage());
	    if (error != Error.OK)
		log.severe("Problem sending (ATTRSET) error back to client - " + Error.GetText(error));
	}
    }
