...
void ConnectionManager::SendConnectRequest(ConnectionState *connState) {
  ...
  if (connState->localAddr.sin_port != 0)
    error = mImpl->comm->Connect(connState->addr, connState->localAddr, connState->timeout, handlerPtr);
  else
    error = mImpl->comm->Connect(connState->addr, connState->timeout, handlerPtr);
