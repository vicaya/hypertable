...
void ScannerTimestampController::remove_update_timestamp(Timestamp &ts) {
  boost::mutex::scoped_lock lock(m_mutex);
  UpdateTimestampContainer::nth_index<1>::type &sorted_index = m_update_timestamps.get<1>();
...
bool ScannerTimestampController::get_oldest_update_timestamp(Timestamp *timestampp) {
  boost::mutex::scoped_lock lock(m_mutex);
  UpdateTimestampContainer::const_iterator iter = m_update_timestamps.begin();
