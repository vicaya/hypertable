...
namespace {
  ...
  const char *help_text_contents[] = {
    ...
    "SELECT CELLS ....... Selects cells from a table and allows filter by value",
  ...
  const char *help_text_select_cells[] = {
    "",
    "SELECT CELLS",
    "======",
    "",
    ...
    "The parser only accepts a single timestamp predicate.  The '=^' operator is the",
    "\"starts with\" operator.  It will return all rows that have the same prefix as",
    "the operand. the SELECT CELLS commands adds the ability to filter by value on top of",
    "the features provided by the SELECT command."
    "",
    "Options",
    "-------",
    "",
    ...
    "    SELECT CELLS * FROM test WHERE (ROW < 'c' or ROW > 'd');",
    "    SELECT CELLS * FROM test WHERE (ROW < 'b' or ROW =^ 'b');",
    "    SELECT CELLS * FROM test WHERE \"farm\",\"tag:abaca\" < CELL <= \"had\",\"tag:abacinate\";",
    "    SELECT CELLS * FROM test WHERE \"farm\",\"tag:abaca\" <= CELL <= \"had\",\"tag:abacinate\";",
    "    SELECT CELLS * FROM test WHERE CELL = \"foo\",\"tag:adactylism\";",
    "    SELECT CELLS * FROM test WHERE CELL =^ \"foo\",\"tag:ac\";",
    "    SELECT CELLS * FROM test WHERE CELL =^ \"foo\",\"tag:a\";",
    "    SELECT CELLS * FROM test WHERE CELL > \"old\",\"tag:abacate\";",
    "    SELECT CELLS * FROM test WHERE CELL >= \"old\",\"tag:abacate\";",
    "    SELECT CELLS * FROM test WHERE \"old\",\"tag:foo\" < CELL >= \"old\",\"tag:abacate\";",
    "    SELECT CELLS * FROM test WHERE ( CELL = \"maui\",\"tag:abaisance\" OR ",
    "                               CELL = \"foo\",\"tag:adage\" OR ",
    "                               CELL = \"cow\",\"tag:Ab\" OR ",
    "                               CELL =^ \"foo\",\"tag:acya\");",
    "    SELECT CELLS * FROM test INTO FILE \"dfs:///tmp/foo\";",
    "    SELECT CELLS col2:\"bird\" from RegexpTest WHERE ROW REGEXP \"http://.*\";",
    "    SELECT CELLS col1:/^w[^a-zA-Z]*$/ from RegexpTest WHERE ROW REGEXP \"m.*\\s\\S\";",
  ...
  HelpTextMap &build_help_text_map() {
    ...
    (*map)["select cells"] = help_text_select_cells;
...
void HqlHelpText::install_range_server_client_text() {
  ...
  text_map["select cells"] = help_text_select_cells;
