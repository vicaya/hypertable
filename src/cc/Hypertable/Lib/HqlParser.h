  ...
  namespace HqlParser {
    ...
    struct create_column_family {
      ...
      void operator()(char const *str, char const *end) const {
        ...
        if (iter != state.cf_map.end())
          HT_THROW(Error::HQL_PARSE_ERROR, std::string("Column family '") + state.cf->name + " multiply defined.");
    ...
    struct add_column_family {
      ...
      void operator()(char const *str, char const *end) const {
        ...
        if (iter == state.cf_map.end())
          HT_THROW(Error::HQL_PARSE_ERROR, std::string("Access Group '") + state.ag->name + "' includes unknown column family '" + name + "'");
        if ((*iter).second->ag != "" && (*iter).second->ag != state.ag->name)
          HT_THROW(Error::HQL_PARSE_ERROR, std::string("Column family '") + name + "' can belong to only one access group");
    ...
    struct set_table_compressor {
      ...
      void operator()(char const *str, char const *end) const {
        ...
        if (state.table_compressor != "")
          HT_THROW(Error::HQL_PARSE_ERROR, std::string("table compressor multiply defined"));
    ...
    struct scan_set_row {
      ...
      void operator()(char const *str, char const *end) const {
        ...
        if (state.scan.row != "")
          HT_THROW(Error::HQL_PARSE_ERROR, std::string("SELECT ROW predicate multiply defined."));
        else if (state.scan.start_row != "" || state.scan.end_row != "")
          HT_THROW(Error::HQL_PARSE_ERROR, std::string("SELECT conflicting row specifications."));
    ...
    struct scan_set_start_row {
      ...
      void operator()(char const *str, char const *end) const {
        ...
        if (state.scan.start_row != "")
          HT_THROW(Error::HQL_PARSE_ERROR, std::string("SELECT row range lower bound specified more than once."));
        state.scan.start_row = std::string(str, end-str);
        trim_if(state.scan.start_row, boost::is_any_of("'\""));
    ...
    struct scan_set_end_row {
      ...
      void operator()(char const *str, char const *end) const {
        ...
        if (state.scan.end_row != "")
          HT_THROW(Error::HQL_PARSE_ERROR, std::string("SELECT row range upper bound specified more than once."));
    ...
    struct scan_set_max_versions {
      ...
      void operator()(const unsigned int &ival) const {
        ...
        if (state.scan.max_versions != 0)
          HT_THROW(Error::HQL_PARSE_ERROR, std::string("SELECT MAX_VERSIONS predicate multiply defined."));
    ...
    struct scan_set_limit {
      ...
      void operator()(const unsigned int &ival) const {
        ...
        if (state.scan.limit != 0)
          HT_THROW(Error::HQL_PARSE_ERROR, std::string("SELECT LIMIT predicate multiply defined."));
    ...
    struct scan_set_outfile {
      ...
      void operator()(char const *str, char const *end) const {
        ...
        if (state.scan.outfile != "")
          HT_THROW(Error::HQL_PARSE_ERROR, std::string("SELECT INTO FILE multiply defined."));
    ...
    struct scan_set_start_time {
      ...
      void operator()(char const *str, char const *end) const {
        ...
        if (state.scan.start_time != 0)
          HT_THROW(Error::HQL_PARSE_ERROR, std::string("SELECT multiple start time predicates."));
        time_t t = timegm(&state.tmval);
        if (t == (time_t)-1)
          HT_THROW(Error::HQL_PARSE_ERROR, std::string("SELECT invalid start time."));
    ...
    struct scan_set_end_time {
      ...
      void operator()(char const *str, char const *end) const {
        ...
        if (state.scan.end_time != 0)
          HT_THROW(Error::HQL_PARSE_ERROR, std::string("SELECT multiple end time predicates."));
        time_t t = timegm(&state.tmval);
        if (t == (time_t)-1)
          HT_THROW(Error::HQL_PARSE_ERROR, std::string("SELECT invalid end time."));
    ...
    struct set_insert_timestamp {
      ...
      void operator()(char const *str, char const *end) const {
        ...
        time_t t = timegm(&state.tmval);
        if (t == (time_t)-1)
          HT_THROW(Error::HQL_PARSE_ERROR, std::string("INSERT invalid timestamp."));
    ...
    struct set_delete_timestamp {
      ...
      void operator()(char const *str, char const *end) const {
        ...
        time_t t = timegm(&state.tmval);
        if (t == (time_t)-1)
          HT_THROW(Error::HQL_PARSE_ERROR, std::string("DELETE invalid timestamp."));
    ...
    struct hql_interpreter : public grammar<hql_interpreter> {
      ...
      struct definition {
        ...
        definition(hql_interpreter const &self)  {
          ...
          date
            = lexeme_d[                limit_d(0u, 9999u)[uint4_p[scan_set_year(self.state)]] >> '-'    //  Year
          ...
          time
            = lexeme_d[                limit_d(0u, 23u)[uint2_p[scan_set_hours(self.state)]] >> ':'       //  Hours 00..23
