...
void init_default_options() {
  ...
  file_desc().add_options()
    ...
    ("Hypertable.Request.Timeout", i32()->default_value(20),
        "Length of time, in seconds, before timing out requests (system wide)")
    ("HdfsBroker.Port", i16(),
    ...
    ("DfsBroker.Timeout", i32()->default_value(60), "Length of time, "
        "in seconds, to wait before timing out DFS Broker requests. This "
        "takes precedence over Hypertable.Request.Timeout")
    ("Hyperspace.Timeout", i32()->default_value(30), "Timeout (sec) for "
        "hyperspace requests (prefered to Hypertable.Request.Timeout")
    ...
    ("Hyperspace.Master.Dir", str(), "Root of hyperspace file and directory "
        ...
        "the installation directory)")
    ("Hyperspace.KeepAlive.Interval", i32()->default_value(10),
        "Hyperspace Keepalive interval (see Chubby paper)")
    ("Hyperspace.Lease.Interval", i32()->default_value(60),
        "Hyperspace Lease interval (see Chubby paper)")
    ("Hyperspace.GracePeriod", i32()->default_value(60),
        "Hyperspace Grace period (see Chubby paper)")
    ("Hypertable.Client.Timeout", i32()->default_value(120), "Timeout in (sec) "
    ...
    ("Hypertable.Master.Timeout", i32()->default_value(30), "Timeout (sec)"
    ...
    ("Hypertable.Master.Gc.Interval", i32()->default_value(300),
        "Garbage collection interval in seconds by Master")
    ("Hypertable.RangeServer.Timeout", i32()->default_value(30), "Timeout (sec)"
    ...
    ("Hypertable.RangeServer.Scanner.Ttl", i32()->default_value(120),
        "Number of seconds of inactivity before destroying scanners")
    ("Hypertable.RangeServer.Timer.Interval", i32()->default_value(60),
        "Timer interval in seconds (reaping scanners, purging commit logs...)")
