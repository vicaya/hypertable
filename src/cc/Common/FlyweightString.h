...
namespace Hypertable {
  ...
  public:
    ~FlyweightString() {
      for (std::set<const char *, lt_cstr>::iterator iter = m_strings.begin(); iter != m_strings.end(); iter++)
	delete [] *iter;
    }
    const char *get(const char *str) {
      std::set<const char *, lt_cstr>::iterator iter = m_strings.find(str);
  ...
  private:
    std::set<const char *, lt_cstr>  m_strings;
  };
