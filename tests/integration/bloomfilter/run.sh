...
restart_servers() {
  ...
  $HT_HOME/bin/start-rangeserver.sh \
...
test() {
  ...
  $HT_HOME/bin/hypertable --no-prompt < $SCRIPT_DIR/create-bloom-${filter_type}-table.hql
  
  echo "================="
  echo "random WRITE test"
  echo "================="
  $HT_HOME/bin/random_write_test --blocksize=100 $DATA_SIZE
  ...
  echo "random READ test"
  echo "================="
  $HT_HOME/bin/random_read_test --blocksize=100 $DATA_SIZE 
}
